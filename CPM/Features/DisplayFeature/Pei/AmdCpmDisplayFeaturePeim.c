 ...
 *
 * Contains code that initialized Display Feature before memory init.
 *
  ...
  )
{
  ...
  CpmTablePpiPtr->CommonFunction.PostCode (0xC003);
  ...
  if (MainTablePtr && PcieTopologyTablePtr && DevicePathTablePtr) {
    if (MainTablePtr->DisplayFeature.Config.PowerXpressFixedMode || MainTablePtr->DisplayFeature.Config.PowerXpressDynamicMode) {
      for (i = 0; i < AMD_DISPLAY_DEVICE_PATH_SIZE; i++) {
        ...
        if (DevicePathTablePtr->Path[i].IsDgpu && DevicePathTablePtr->Path[i].FeatureMask.Mask.PowerXpress) {
  ...
  CpmTablePpiPtr->CommonFunction.PostCode (0xC103);
