 ...
 *
 * @param[in] AmdFchConfig  Fch configuration structure pointer.
 */
VOID
FchHwmInit (
  ...
  )
{
  ...
  HwmMemRw ( EfiPciWidthUint32, MmioAddr + MISC_BASE + FCH_MISC_REG50, ~(UINT32) (BIT11 + BIT20), (UINT32) (BIT11 + BIT20));
  ...
  if (FchConfig->FchHwm.FanLinearEnhanceEn == 0) {
    HwmMemRw ( EfiPciWidthUint32, MmioAddr + MISC_BASE + FCH_MISC_REG50, ~(UINT32)BIT11, (UINT32)BIT11);
    ...
    HwmMemRw ( EfiPciWidthUint32, MmioAddr + MISC_BASE + FCH_MISC_REG50, ~(UINT32)BIT11, 0);
  }
  //check for fanLinearHoldFix
  if (FchConfig->FchHwm.FanLinearHoldFix == 0) {
    HwmMemRw ( EfiPciWidthUint32, MmioAddr + MISC_BASE + FCH_MISC_REG50, ~(UINT32)BIT20, BIT20);
  } else {
    HwmMemRw ( EfiPciWidthUint32, MmioAddr + MISC_BASE + FCH_MISC_REG50, ~(UINT32)BIT20, 0);
  }
}
  ...
  )
{
  ...
  if (FchConfig->FchHwm.HwmFchtsiAutoPoll == 1) {
    ...
    HwmMemRw ( EfiPciWidthUint8, MmioAddr + PMIO_BASE + FCH_PMIOA_REG2E, ~(UINT32) (BIT1 + BIT2), BIT2);
    ...
    //
    HwmMemRw ( EfiPciWidthUint8, MmioAddr + PMIO2_BASE + FCH_PMIO2_REG92, ~(UINT32)BIT3, BIT3);
    ValueByte = 0x00;
  ...
  )
{
  ...
  if ( FchConfig->FchHwm.HwMonitorEnable ) {
    ...
    LibFchIoWrite (EfiPciWidthUint8, SmbusBase + FCH_SMB_IOREG14, &ValueByte);
    FchHwmFchtsiAutoPollingPause (FchConfig);
    HwmMemRw ( EfiPciWidthUint8, MmioAddr + PMIO_BASE + FCH_PMIOA_REG2E, ~(UINT32) (BIT1 + BIT2), 0);
    HwmMemRw ( EfiPciWidthUint8, MmioAddr + PMIO2_BASE + FCH_PMIO2_REG92, ~(UINT32)BIT3, 0x00);
