...
#include <Ppi\AmdInitResetPpi.h>
#include <Ppi\AmdInitEarlyPpi.h>
#include <Ppi\AmdInitPostPpi.h>
#include <Ppi\AgesaMemPpi.h>
...
--*/
{
  ...
  Status = OemAgesaGetS3Info (
              PeiServices,
              S3DataType,
              (UINTN *)&AmdS3LateParamsPtr->S3DataBlock.VolatileStorageSize,
  ...
  )
{
  ...
  Status = OemAgesaGetS3Info (
              PeiServices,
              S3DataType,
              (UINTN *)&AmdResumeParamsPtr->S3DataBlock.NvStorageSize,
  ...
  )
{
  ...
  ZeroMem (&AmdResetParams, sizeof (AMD_RESET_PARAMS));
  //
...
--*/
{
  ...
  Status = AmdCpuInitialize ((EFI_PEI_SERVICES **)PeiServices);
