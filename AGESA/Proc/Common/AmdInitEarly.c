  ...
  )
{
  ...
  // IMPORTANT: All AP cores call Topology Init.
  IDS_PERF_TIMESTAMP (TP_BEGINAMDTOPOINITIALIZE, &EarlyParams->StdHeader);
  IDS_HDT_CONSOLE (MAIN_FLOW, "AmdTopologyInitialize: Start\n");
  CalledAgesaStatus = AmdTopologyInitialize (&EarlyParams->StdHeader, &EarlyParams->PlatformConfig);
  IDS_HDT_CONSOLE (MAIN_FLOW, "AmdTopologyInitialize: End\n");
  IDS_PERF_TIMESTAMP (TP_ENDAMDTOPOINITIALIZE, &EarlyParams->StdHeader);
  if (CalledAgesaStatus > EarlyInitStatus) {
    EarlyInitStatus = CalledAgesaStatus;
  }
