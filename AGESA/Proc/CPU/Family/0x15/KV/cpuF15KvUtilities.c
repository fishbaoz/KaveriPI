 ...
 * @file
 *
 * AMD Family_15 models 30h - 3Fh specific utility functions.
 *
 * Provides numerous utility functions specific to family 15h KV.
 *
...
CODE_GROUP (G2_PEI)
RDATA_GROUP (G2_PEI)
#define FILECODE PROC_CPU_FAMILY_0X15_KV_CPUF15KVUTILITIES_FILECODE
...
VOID
STATIC
F15KvNbPstateDisCore (
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

VOID
STATIC
F15KvGetNbFreqNumeratorInMHz (
  IN       UINT32                NbFid,
...
AGESA_STATUS
STATIC
F15KvCalculateNbFrequencyInMHz (
  IN       UINT32                NbFid,
  IN       UINT32                NbDid,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvGetProcIddMax - P%d\n", Pstate);
  ...
  if (PstateMsr.PsEnable == 1) {
    F15KvCmnCalculateCurrentInmA ((UINT32) PstateMsr.IddValue, (UINT32) PstateMsr.IddDiv, &ProcIddMaxPerCore, StdHeader);
...
/*---------------------------------------------------------------------------------------*/
/**
 * Set down core register on Kaveri
 *
...
CONST CPU_CORE_LEVELING_FAMILY_SERVICES ROMDATA F15KvCoreLeveling =
{
  0,
  F15KvSetDownCoreRegister
};
  ...
  )
{
  ...
  for (i = 0; i <= NbPsMaxVal; i++) {
    ...
    if (((NB_PSTATE_REGISTER *) &LocalPciRegister)->NbPstateEn == 1) {
      AgesaStatus = F15KvCalculateNbFrequencyInMHz (((NB_PSTATE_REGISTER *) &LocalPciRegister)->NbFid,
  ...
  if (i > NbPsMaxVal) {
    ...
    LibAmdPciRead (AccessWidth32, *PciAddress, &LocalPciRegister, StdHeader);
    F15KvCalculateNbFrequencyInMHz (((NB_PSTATE_STS_REGISTER *) &LocalPciRegister)->CurNbFid,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvGetNbPstateInfo - NB P%d\n", NbPstate);
  ...
  if ((NbPstate == 0) || (FamilySpecificServices->IsNbPstateEnabled (FamilySpecificServices, PlatformConfig, StdHeader))) {
    ...
    if (NbPstate <= NbPstateCtrlReg.NbPstateMaxVal) {
      ...
      IDS_HDT_CONSOLE (CPU_TRACE, "    En:%d Fid:%x Did:%x Vid:%x\n", NbPstateReg.NbPstateEn, NbPstateReg.NbFid, NbPstateReg.NbDid, GetF15KvNbVid (&NbPstateReg));
      ...
      if (NbPstateReg.NbPstateEn == 1) {
        ...
        F15KvGetNbFreqNumeratorInMHz (NbPstateReg.NbFid, FreqNumeratorInMHz, StdHeader);
        F15KvGetNbFreqDivisor (NbPstateReg.NbDid, FreqDivisor, StdHeader);
        // Check for P-state Bandwidth Requirements on
        // "NBCOF >= 700MHz"
        ASSERT ((*FreqNumeratorInMHz / *FreqDivisor) >= 700);

        NbVid = GetF15KvNbVid (&NbPstateReg);
        F15KvCovertVidInuV  (NbVid, VoltageInuV, StdHeader);
        PstateIsValid = TRUE;
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvGetNbIddMax - NB P%d\n", NbPstate);
  ...
  if (NbPstate <= NbPstateCtrlReg.NbPstateMaxVal) {
    ...
    // Ensure that requested NbPstate is enabled
    if (NbPstateReg.NbPstateEn == 1) {
      F15KvCmnCalculateCurrentInmA (NbPstateReg.NbIddValue, NbPstateReg.NbIddDiv, NbIddMax, StdHeader);
      IsNbPsEnabled = TRUE;
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvNbPstateDis\n");

  // Set NbPstateFidVidSbcEn to allow NB P-state transitions
  PciAddress.AddressValue = NB_PSTATE_CTRL_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &NbPsCtrlOnEntry, StdHeader);
  ((NB_PSTATE_CTRL_REGISTER *) &NbPsCtrlOnEntry)->NbPstateFidVidSbcEn = 1;

  LibAmdPciWrite (AccessWidth32, PciAddress, &NbPsCtrlOnEntry, StdHeader);
  ...
  if (!FamilySpecificServices->IsNbPstateEnabled (FamilySpecificServices, &CpuEarlyParamsPtr->PlatformConfig, StdHeader)) {
    ...
    if (((((NB_PSTATE_STS_REGISTER *) &PciData)->StartupNbPstate != ((NB_PSTATE_CTRL_REGISTER *) &NbPsCtrl)->NbPstateHi) &&
        ...
        ((NB_PSTATE_STS_REGISTER *) &PciData)->CurNbPstateLo == 1)  {
      SkipNbPsLoPart = TRUE;
    }
    ...
    TaskPtr.FuncAddress.PfApTask = F15KvNbPstateDisCore;
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvNbPstateDisCore\n");
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvGetNbFreqNumeratorInMHz - NbFid=%d\n", NbFid);
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvGetNbFreqDivisor - NbDid=%d\n", NbDid);
  *FreqDivisor = (1 << NbDid);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvCalculateNbFrequencyInMHz - NbFid=%x, NbDid=%x\n", NbFid, NbDid);

  ReturnStatus = AGESA_SUCCESS;
  F15KvGetNbFreqNumeratorInMHz (NbFid, &FreqNumeratorInMHz, StdHeader);
  F15KvGetNbFreqDivisor (NbDid, &FreqDivisor, StdHeader);
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvCovertVidInuV\n");
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvCmnGetIddDivisor - IddDiv=%d\n", IddDiv);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvCmnCalculateCurrentInmA - IddValue=%x, IddDiv=%x\n", IddValue, IddDiv);

  F15KvCmnGetIddDivisor (IddDiv, &IddDivisor, StdHeader);
...
/*---------------------------------------------------------------------------------------*/
/**
 * Set trim values from fuses.
 *   - read fuses at D0F0xBC_xC01040A0
 *   - D18F5x188[NbOffsetTrim] = D0F0xBC_xC01040A0[SviLoadLineOffsetVddNb]
 *   - D18F5x188[NbLoadLineTrim] = D0F0xBC_xC01040A0[SviLoadLineTrimVddNb]
 *   - D18F5x12C[CoreOffsetTrim] = D0F0xBC_xC01040A0[SviLoadLineOffsetVdd]
 *   - D18F5x12C[CoreLoadLineTrim] = D0F0xBC_xC01040A0[SviLoadLineTrimVdd]
 *
 * @param[in]     FamilySpecificServices  The current Family Specific Services
 * @param[in]     CpuEarlyParamsPtr       Service Parameters
 * @param[in]     StdHeader               Handle of Header for calling lib functions and services.
...
{
  D0F0xBC_xC01040A0_STRUCT       D0F0xBC_xC01040A0;
  PCI_ADDR                       PciAddress;
  CLK_PWR_TIMING_CTRL_5_REGISTER ClockPowerTimingControl5;
  CLK_PWR_TIMING_CTRL_4_REGISTER ClockPowerTimingControl4;
  CLK_PWR_TIMING_CTRL1_REGISTER  ClockPowerTimingControl1;

  GnbRegisterReadKV (GnbGetHandle (StdHeader), D0F0xBC_xC01040A0_TYPE, D0F0xBC_xC01040A0_ADDRESS, &D0F0xBC_xC01040A0, 0, StdHeader);
  PciAddress.AddressValue = CPTC5_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &ClockPowerTimingControl5, StdHeader);
  ClockPowerTimingControl5.NbOffsetTrim = D0F0xBC_xC01040A0.Field.SviLoadLineOffsetVddNb_1_0;
  ClockPowerTimingControl5.NbLoadLineTrim = D0F0xBC_xC01040A0.Field.SviLoadLineTrimVddNb_2_0;
  LibAmdPciWrite (AccessWidth32, PciAddress, &ClockPowerTimingControl5, StdHeader);

  PciAddress.AddressValue = CPTC1_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &ClockPowerTimingControl1, StdHeader);

  PciAddress.AddressValue = CPTC4_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &ClockPowerTimingControl4, StdHeader);
  if (ClockPowerTimingControl1.Reserved2 == 0) {
    ClockPowerTimingControl4.CoreOffsetTrim = D0F0xBC_xC01040A0.Field.SviLoadLineOffsetVdd_1_0;
  }
  ClockPowerTimingControl4.CoreLoadLineTrim = D0F0xBC_xC01040A0.Field.SviLoadLineTrimVdd_2_0;
  LibAmdPciWrite (AccessWidth32, PciAddress, &ClockPowerTimingControl4, StdHeader);
}


/*---------------------------------------------------------------------------------------*/
/**
 * Apply a workaround for P-state settings.
 *   - Search all hardware P-states for a 2GHz P-state
 *   - Reduce any 2GHz P-states to 1.9GHz
 *
 * @param[in]     FamilySpecificServices  The current Family Specific Services
 * @param[in]     CpuEarlyParamsPtr       Service Parameters
 * @param[in]     StdHeader               Handle of Header for calling lib functions and services.
  ...
  )
{
  ...
  for (PstateIndex = PS_REG_BASE; PstateIndex <= PS_MAX_REG; PstateIndex++) {
    LibAmdMsrRead (PstateIndex, (UINT64 *) &PstateMsr, StdHeader);
    if (((PstateMsr.CpuFid == 4) && (PstateMsr.CpuDid == 0)) ||
        ((PstateMsr.CpuFid == 24) && (PstateMsr.CpuDid == 1))) {
      PstateMsr.CpuFid = PstateMsr.CpuFid - (UINT32) (1 << PstateMsr.CpuDid);
      LibAmdMsrWrite (PstateIndex, (UINT64 *) &PstateMsr, StdHeader);
    }
  }
}
