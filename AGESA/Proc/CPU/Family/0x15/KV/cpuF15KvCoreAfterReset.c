 ...
 * @file
 *
 * AMD Family_15 Kaveri after warm reset sequence for core P-states
 *
...
#define FILECODE PROC_CPU_FAMILY_0X15_KV_CPUF15KVCOREAFTERRESET_FILECODE
...
VOID
STATIC
F15KvPmCoreAfterResetPhase1OnCore (
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

VOID
STATIC
F15KvPmCoreAfterResetPhase2OnCore (
  IN       VOID              *HwPsMaxVal,
  IN       AMD_CONFIG_PARAMS *StdHeader
  );
...
/*---------------------------------------------------------------------------------------*/
/**
 * Family 15h Kaveri core 0 entry point for performing the necessary steps for core
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvPmCoreAfterReset\n");
  ...
  HwPsMaxVal = ((F15_KV_CLK_PWR_TIMING_CTRL2_REGISTER *) &HwPsMaxVal)->HwPstateMaxVal;

  // Launch each local core to perform steps 1 through 3.
  TaskPtr.FuncAddress.PfApTask = F15KvPmCoreAfterResetPhase1OnCore;
  ...
  TaskPtr.FuncAddress.PfApTaskI = F15KvPmCoreAfterResetPhase2OnCore;
...
/*---------------------------------------------------------------------------------------*/
/**
 * Support routine for F15KvPmCoreAfterReset to perform MSR initialization on all
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvPmCoreAfterResetPhase1OnCore\n");
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvPmCoreAfterResetPhase2OnCore\n");
  ...
  if (((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstateLimit != ((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstate) {
    do {
      LibAmdMsrRead (MSR_COFVID_STS, &LocalMsrRegister, StdHeader);
    } while (GetF15KvCurCpuVid (&LocalMsrRegister) != GetF15KvCpuVid (&TargetPsMsr));
  }
