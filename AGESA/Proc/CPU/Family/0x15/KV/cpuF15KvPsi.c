 ...
 * @file
 *
 * AMD Family_15 KV PSI Initialization
 *
...
#define FILECODE PROC_CPU_FAMILY_0X15_KV_CPUF15KVPSI_FILECODE
  ...
  )
{
  ...
  if ((EntryPoint & (CPU_FEAT_AFTER_POST_MTRR_SYNC | CPU_FEAT_AFTER_RESUME_MTRR_SYNC)) != 0) {
    ...
    IDS_HDT_CONSOLE (CPU_TRACE, "  F15KvPmVrmLowPowerModeEnable\n");
    ...
    if (PlatformConfig->VrmProperties[NbVrm].LowPowerThreshold != 0) {
      ...
      for (NbPstate = 0; NbPstate <= NbPstateMaxVal; NbPstate++) {
        ...
        CurrentNbVid = (UINT32) GetF15KvNbVid (&NbPstateReg);
...
CONST PSI_FAMILY_SERVICES ROMDATA F15KvPsiSupport =
{
  0,
