  ...
  )
{
  ...
  PciAddress.AddressValue = MAKE_SBDFO (0, 0 , PCI_DEV_BASE, FUNC_0, LINK_INIT_CTRL);
  LibAmdPciRead (AccessWidth32, PciAddress, &PciData, StdHeader);

  // bit[5] - indicate a warm reset is or is not required
  PciData &= ~(LINK_INIT_BIOS_RST_DET_0);
  PciData = PciData | (Request->RequestBit << 5);

  // bit[10,9] - indicate warm reset status and count
  PciData &= ~(LINK_INIT_BIOS_RST_DET_1 | LINK_INIT_BIOS_RST_DET_2);
  ...
  )
{
  ...
  // bit[5] - indicate a warm reset is or is not required
  Request->RequestBit = (UINT8) ((PciData & LINK_INIT_BIOS_RST_DET_0) >> 5);
  // bit[10,9] - indicate warm reset status and count
  Request->StateBits = (UINT8) ((PciData & (LINK_INIT_BIOS_RST_DET_1 | LINK_INIT_BIOS_RST_DET_2)) >> 9);
}
