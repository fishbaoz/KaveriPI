  ...
  )
{
  ...
  if (!IsBsp (StdHeader, &IgnoredStatus)) {
    ...
    // Since pre-heap, get compute unit status from hardware
    ...
    LibAmdPciReadBits (PciAddress, 7, 0, &Enabled, StdHeader);
    LibAmdPciReadBits (PciAddress, 23, 16, &DualCore, StdHeader);
    ...
    if ((Enabled != 0) && (ComputeUnitMap != NULL)) {
      while (ComputeUnitMap->Mapping != MaxComputeUnitMapping) {
        if ((Enabled == ComputeUnitMap->Enabled) && (DualCore == ComputeUnitMap->DualCore)) {
          break;
        }
        ComputeUnitMap++;
      }
