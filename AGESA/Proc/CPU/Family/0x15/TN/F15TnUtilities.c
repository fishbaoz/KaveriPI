 ...
 *----------------------------------------------------------------------------------------
 */
extern F15_TN_ES_NB_PSTATE F15TnEarlySampleNbPstateSupport;
extern F15_TN_ES_MCU_PATCH F15TnEarlySampleLoadMcuPatch;
  ...
  )
{
  ...
  if (!SkipHwCfg) {
    if (!(F15TnEarlySampleNbPstateSupport.F15TnEsIsNbPsateEn (StdHeader))) {
      return FALSE;
    }
  }
  ...
  )
{
  ...
  if (!FamilySpecificServices->IsNbPstateEnabled (FamilySpecificServices, &CpuEarlyParamsPtr->PlatformConfig, StdHeader)) {
    ...
    LibAmdMsrRead (MSR_COFVID_CTL, &CofvidCtl, StdHeader);

    SkipNbPsLoPart = (F15TnEarlySampleNbPstateSupport.F15TnEsSkipNbPsLo (&PciData, &NbPsCtrl, StdHeader) || (((COFVID_CTRL_MSR *) &CofvidCtl)->NbPstate == 1)) ? TRUE : FALSE;
 ...
 *
 * This function performs all of the necessary actions to load a microcode patch.
 *
 *  @param[in]   StdHeader           Config handle for library and services.
 *
 */
VOID
F15TnLoadMicrocodePatch (
  IN       AMD_CONFIG_PARAMS      *StdHeader
  )
{
  ...
  F15TnEarlySampleLoadMcuPatch.F15TnUpdateMcuPatchHook (StdHeader);
