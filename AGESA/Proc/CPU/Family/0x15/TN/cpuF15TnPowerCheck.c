  ...
  )
{
  ...
  PsMaxValOnEntry = (UINT8) ClkPwrTimingCtrl2.PstateMaxVal;
  ...
  if (AllowablePstateFound) {
    ...
    if (NumberOfDisabledBoostStates != 0) {
      ...
      for (i = 0; i < NumberOfDisabledBoostStates; i++) {
        F15TnPmPwrChkCopyPstate (i, NumberOfDisabledBoostStates, StdHeader);
      }
    }
  } else {
    ...
    if (NumberOfDisabledSoftwarePstates != 0) {
      ...
      ApTask.FuncAddress.PfApTaskI = F15TnTransitionPstateCore;
      ...
      if (((PSTATE_MSR *) &LocalMsrRegister)->NbPstate == 1) {
        ...
        F15TnNbPstateDis (FamilySpecificServices, CpuEarlyParams, StdHeader);
      }
      ...
      for (i = NumberOfBoostStates; (i + NumberOfDisabledSoftwarePstates) < NM_PS_REG; i++) {
        F15TnPmPwrChkCopyPstate (i, (i + NumberOfDisabledSoftwarePstates), StdHeader);
      }
      ...
      // Adjust the values of HTC / STC limits and PstateMaxVal since software P-states have shifted
      ...
      ClkPwrTimingCtrl2.PstateMaxVal = (ClkPwrTimingCtrl2.PstateMaxVal > NumberOfDisabledSoftwarePstates) ? ClkPwrTimingCtrl2.PstateMaxVal - NumberOfDisabledSoftwarePstates : NumberOfBoostStates;
  ...
  if (LocalPciRegister != 0) {
    ...
    if (PsMaxValOnEntry != InitialPsMaxVal) {
      // Pstates have previously been disabled.  PstateMaxVal persists, so use it.
      HwPstateMaxValAdjusted = TRUE;
      PciAddress.AddressValue = CPTC2_PCI_ADDR;
      LibAmdPciRead (AccessWidth32, PciAddress, &LocalPciRegister, StdHeader);
      LocalPciRegister = ClkPwrTimingCtrl2.PstateMaxVal;
  ...
  // Apply the BKDG recommendation for PstateMaxVal here to close the window in which a warm reset could cause a logic problem
