 ...
 *----------------------------------------------------------------------------------------
 */
#define F15_MMIO_ALIGN 0x10000l
  ...
  )
{
  ...
  FragmentMmioRange.Limit = F15_MMIO_ALIGN;
  ...
  for (MmioPair = 0; MmioPair < MMIO_REG_PAIR_NUM; MmioPair++) {
    ...
    MmioRange[MmioPair].Limit = ((MmioLimitLow.MmioLimit << 16) | (((UINT64) MmioBaseLimitHi.MmioLimit) << 40)) + F15_MMIO_ALIGN;
  ...
  if (ConfMapRange == CONF_MAP_NUM) {
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, " [ERROR] AmdAddMmioParams.TargetAddress doesn't belong to any node.\n");
...
//                                                                                   new base   new limit
          ...
          for (i = 1; NewMmioRange.Limit >= MmioRange[MmioPair + i].Base; i++) {
            if ((NewMmioRange.Attribute.MmioPostedRange == MmioRange[MmioPair + i].Attribute.MmioPostedRange) &&
              ...
              MmioRange[MmioPair + i].Limit = F15_MMIO_ALIGN;
      ...
      for (MmioPair = 0; MmioPair < MMIO_REG_PAIR_NUM; MmioPair++) {
        if ((MmioRange[MmioPair].Modified == TRUE) || (NewMmioRange.Limit < MmioRange[MmioPair].Base) || (NewMmioRange.Base > MmioRange[MmioPair].Limit)) {
          ...
          if ((NewMmioRange.Base >= MmioRange[MmioPair].Base) && (NewMmioRange.Limit <= MmioRange[MmioPair].Limit)) {
            if ((MmioRange[MmioPair].Limit - NewMmioRange.Limit) >= F15_MMIO_ALIGN) {
            ...
            if ((NewMmioRange.Base -  MmioRange[MmioPair].Base) < F15_MMIO_ALIGN) {
          ...
          if ((NewMmioRange.Base <= MmioRange[MmioPair].Base) && (NewMmioRange.Limit >= MmioRange[MmioPair].Limit)) {
            ...
            MmioRange[MmioPair].Limit = F15_MMIO_ALIGN;
            MmioRange[MmioPair].Attribute.MmioReadableRange = 0;
            MmioRange[MmioPair].Attribute.MmioWritableRange = 0;
