  ...
  )
{
  ...
  if (!IsWarmReset (StdHeader)) {
    ...
    TaskPtr.FuncAddress.PfApTask = EnableLowPwrPstateOnSocket;
    ...
    EnableLowPwrPstateOnSocket (StdHeader);
  } else {
    TaskPtr.FuncAddress.PfApTask = ProgramHtcPstateLimit;
    TaskPtr.DataTransfer.DataSizeInDwords = 0;
    TaskPtr.ExeFlags = WAIT_FOR_CORE;
    ...
    ProgramHtcPstateLimit (StdHeader);
  }
  ...
  )
{
  ...
  // F3x64 should only be programmed on internal node 0 (F3xE8[IntNodeNum]=00b) if F3xE8[MultiNodeCpu]=1.
