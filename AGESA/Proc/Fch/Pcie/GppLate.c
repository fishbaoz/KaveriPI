  ...
  )
{
  ...
  if ( ReadFchChipsetRevision ( StdHeader ) >= FCH_BOLTON ) {
    ...
    if (PortCfg->PortDetected) {
      RwAlink (FCH_RCINDXC_REG10, 0xFFFFFFFE, 0, StdHeader);
      ReadPci (PCI_ADDRESS (0, 21, 0, 0x5B), AccessWidth8, &BusNum, StdHeader);
      BusNum |= BIT0;
      WritePci (PCI_ADDRESS (0, 21, 0, 0x5B), AccessWidth8, &BusNum, StdHeader);
      RwAlink (FCH_RCINDXC_REG10, 0xFFFFFFFE, BIT0, StdHeader);
    }
  }
  //
