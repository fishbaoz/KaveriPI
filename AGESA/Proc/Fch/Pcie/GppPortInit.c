  ...
  )
{
  ...
  if ((ReadFchSleepType (StdHeader) == ACPI_SLPTYP_S3) && ( FchGpp->IsCapsuleMode == FALSE )) {
  ...
  )
{
  if (FchGpp->PcieAer) {
    ...
    //
    RwAlink (FCH_ABCFG_REG310 | (UINT32) (ABCFG << 29), ~(UINT32) (BIT7 + BIT4), BIT28 + BIT27 + BIT26 + BIT1, StdHeader);
  ...
  )
{
  ...
  //
  // For PMI_CAP.PME_Support field programming
  //
  RwAlink (FCH_ABCFG_REG314 | (UINT32) (ABCFG << 29), ~(UINT32) (0x1f << 10), (UINT32) (0x19 << 10), StdHeader);
  //
  // Initialize and configure GPP
  //
  if (FchGpp->GppFunctionEnable) {
    if (( FchGpp->NewGppAlgorithm == FALSE ) || (( (ReadFchSleepType (StdHeader) != ACPI_SLPTYP_S3) ) || ( FchGpp->IsCapsuleMode == TRUE ))) {
    ...
    if ( FchGpp->GppMemWrImprove == TRUE ) {
      ...
      RwAlink (FCH_ABCFG_REG54 | (UINT32) (ABCFG << 29), ~(UINT32) BIT26, 0, StdHeader);
      RwAlink (FCH_RCINDXC_REG10, ~(UINT32) (BIT12 + BIT11 + BIT10), 0, StdHeader);
    }
    ...
    if ( FchGpp->NewGppAlgorithm == TRUE ) {
      ...
      while (ResetCounter--) {
        ...
        } else if ((FailPorts != 0) && ((ReadFchSleepType (StdHeader) != ACPI_SLPTYP_S3) || ( FchGpp->IsCapsuleMode == TRUE ))) {
          ProgramGppTogglePcieReset (FchGpp->GppToggleReset, StdHeader);
        } else {
          break;
        }
