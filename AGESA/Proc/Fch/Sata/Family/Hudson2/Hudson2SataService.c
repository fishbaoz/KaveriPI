  ...
  )
{
  ...
  if (LocalCfgPtr->Sata.SataMode.IdeEnable == DISABLED) {
    ...
    if (LocalCfgPtr->Sata.SataMode.Sata6AhciCap == ENABLED) {
      ...
      RwMem ((Bar5 + FCH_SATA_BAR5_REG00), AccessWidth8, ~(UINT32) (BIT2 + BIT1 + BIT0), BIT2 + BIT0);
      RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG50 + 2), AccessWidth8, ~(UINT32) (BIT3 + BIT2 + BIT1), BIT2 + BIT1, StdHeader);
      ...
      if ( LocalCfgPtr->Sata.SataCccSupport ) {
        ...
        RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG50 + 2), AccessWidth8, ~(UINT32) (BIT3 + BIT2 + BIT1), BIT2, StdHeader);
      }
    }
  } else {
    //
    ...
    //
    RwMem ((Bar5 + FCH_SATA_BAR5_REG00), AccessWidth8, ~(UINT32) (BIT2 + BIT1 + BIT0), BIT2 + BIT1 + BIT0);
    RwMem ((Bar5 + FCH_SATA_BAR5_REG0C), AccessWidth8, 0x00, 0xFF);
    if ( LocalCfgPtr->Sata.SataCccSupport ) {
      RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG50 + 2), AccessWidth8, ~(UINT32) (BIT3 + BIT2 + BIT1), BIT3, StdHeader);
  ...
  )
{
  ...
  if ( FchSataHotRemovalEnh ) {
    RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG80), AccessWidth16, ~(UINT32) BIT8, BIT8, StdHeader);
    RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REGA8), AccessWidth16, ~(UINT32) BIT0, BIT0, StdHeader);
    RwPci (((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG80), AccessWidth16, ~(UINT32) BIT8, 0, StdHeader);
  }
}
