  ...
  )
{
  ...
  RwPci ((UINT32) Value + FCH_OHCI_REG50, AccessWidth8, ~(UINT32) (BIT0 + BIT5 + BIT12), 0, FchDataPtr->StdHeader);
  //
  // USB SMI Handshake
  //
  RwPci ((UINT32) Value + FCH_OHCI_REG50 + 1, AccessWidth8, ~(UINT32) BIT4, 0x00, FchDataPtr->StdHeader);
  ...
  //
  RwPci ((UINT32) Value + FCH_OHCI_REG50 + 3, AccessWidth8, ~(UINT32) BIT6, BIT6, FchDataPtr->StdHeader);
  //
  // L1 Early Exit
  ...
  //
  RwPci ((UINT32) Value + FCH_OHCI_REG50 + 2, AccessWidth8, ~(UINT32) (BIT3 + BIT4 + BIT6 + BIT7), BIT3 + BIT4 + BIT6 + BIT7, FchDataPtr->StdHeader);
  if ( FchDataPtr->Usb.UsbMsiEnable) {
    RwPci ((UINT32) Value + FCH_OHCI_REG40 + 1, AccessWidth8, ~(UINT32) BIT0, 0x00, FchDataPtr->StdHeader);
    RwPci ((UINT32) Value + FCH_OHCI_REG50, AccessWidth8, ~(UINT32) BIT5, BIT5, FchDataPtr->StdHeader);
  }
  ...
  // SB02643
  RwMem (ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REGB4, AccessWidth8, ~(UINT32) BIT7, BIT7);
  // RPR 7.27 SB02686
  RwMem (ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REGED, AccessWidth8, ~(UINT32) BIT2, BIT2);
  // SB02698
  RwPci ((UINT32) Value + FCH_OHCI_REG50, AccessWidth8, ~(UINT32) BIT0, 0, FchDataPtr->StdHeader);
  if ( ReadFchChipsetRevision ( FchDataPtr->StdHeader ) >= FCH_BOLTON ) {
    // RPR 7.44 Enhance OHCI RxCrc16Match sync logic
    // RwPci ((UINT32) Value + FCH_OHCI_REG80, AccessWidth32, (UINT32) (~(0x01 << 11)), (UINT32) (0x01 << 11), FchDataPtr->StdHeader);
    // RPR 7.44 Enhance EHCI/OHCI resume/disconnect detection timer
    RwPci ((UINT32) Value + FCH_OHCI_REG80, AccessWidth32, (UINT32) (~(0x01 << 12)), (UINT32) (0x01 << 12), FchDataPtr->StdHeader);
    // RPR 7.45 Enhance EHCI/OHCI hold resume
    RwPci ((UINT32) Value + FCH_OHCI_REG80, AccessWidth32, (UINT32) (~(0x03 << 14)), (UINT32) (0x03 << 14), FchDataPtr->StdHeader);
    // UBTS 422214 False S3 resume when port owned by OHCI A1
    RwPmio (FCH_PMIOA_REGF4, AccessWidth8, ~(UINT32) BIT2, BIT2, FchDataPtr->StdHeader);
  }
}
