...
#define FILECODE PROC_RECOVERY_TOPOLOGY_TOPOINITRECOVERY_FILECODE
    ...
    *SocketDieToNodeMap = (SOCKET_DIE_TO_NODE_MAP)AllocHeapParams.BufferPtr;
    ...
    for (i = 0; i < MAX_SOCKETS; i++) {
      for (j = 0; j < MAX_DIES; j++) {
        (**SocketDieToNodeMap)[i][j].Node = TOPOLOGY_LIST_TERMINAL;
        (**SocketDieToNodeMap)[i][j].LowCore = TOPOLOGY_LIST_TERMINAL;
        (**SocketDieToNodeMap)[i][j].HighCore = TOPOLOGY_LIST_TERMINAL;
      }
    }
  }
  // Allocate heap for the table
  ...
  if (HeapAllocateBuffer (&AllocHeapParams, StdHeader) == AGESA_SUCCESS) {
    ...
    *NodeToSocketDieMap = (NODE_TO_SOCKET_DIE_MAP)AllocHeapParams.BufferPtr;
    ...
    for (i = 0; i < MAX_NODES; i++) {
      (**NodeToSocketDieMap)[i].Socket = TOPOLOGY_LIST_TERMINAL;
      (**NodeToSocketDieMap)[i].Die = TOPOLOGY_LIST_TERMINAL;
    }
  }
}
