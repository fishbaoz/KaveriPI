  ...
  )
{
  ...
  if (LinkAspm.DownstreamAspm != AspmL1) {
    ...
    // Software must sequence the enabling and disabling of ASPM L1 such that the Upstream component
    // is enabled before the Downstream component and disabled after the Downstream component.
    // Enable  ASPM  Upstream component
    PcieAspmEnableOnDevice (Upstream, LinkAspm.UpstreamAspm, StdHeader);
    // Enable  ASPM  Donstream component
    PcieAspmEnableOnFunction (Downstream, LinkAspm.DownstreamAspm, StdHeader);
