 ...
 */
AGESA_STATUS
PcieConfigureBridgeResourcesV1 (
  IN      PCI_ADDR             Port,
  IN      AMD_CONFIG_PARAMS    *StdHeader
  );

VOID
PcieFreeBridgeResourcesV1 (
  IN      PCI_ADDR             Port,
  IN      AMD_CONFIG_PARAMS    *StdHeader
  );

GFX_WORKAROUND_STATUS
PcieDeskewWorkaroundV1 (
  IN      PCI_ADDR            Device,
  IN      AMD_CONFIG_PARAMS   *StdHeader
  );
...
VOID
PcieProgramCpuMmioV1 (
  OUT    UINT32              *SaveValues,
  IN     AMD_CONFIG_PARAMS   *StdHeader
  );

VOID
PcieRestoreCpuMmioV1 (
  IN     UINT32              *RestoreValues,
  IN     AMD_CONFIG_PARAMS   *StdHeader
  );

BOOLEAN
PcieIsDeskewCardDetectedV1 (
  IN      UINT16             DeviceId
  );
  ...
  )
{
  ...
  if (PcieConfigureBridgeResourcesV1 (Port, StdHeader) == AGESA_SUCCESS) {
    ...
    if (DeviceId != 0xffff) {
      ...
      if (VendorId != 0xffff) {
        ...
        Status = GFX_WORKAROUND_SUCCESS;
        if (DevClassCode == 3) {
          PcieProgramCpuMmioV1 (SaveValueData, StdHeader);
          if (VendorId == 0x1002 && PcieIsDeskewCardDetectedV1 (DeviceId)) {
            Status = PcieDeskewWorkaroundV1 (Ep, StdHeader);
          } else if (VendorId == 0x10DE) {
            Status = PcieNvWorkaroundV1 (Ep, StdHeader);
          }
          PcieRestoreCpuMmioV1 (SaveValueData, StdHeader);
        }
      }
    }
    PcieFreeBridgeResourcesV1 (Port, StdHeader);
  }
  return Status;
}
