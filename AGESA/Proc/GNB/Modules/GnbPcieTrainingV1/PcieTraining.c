  ...
  )
{
  ...
  if (Engine->Type.Port.PortData.ResetId == ResetId && !PcieConfigIsSbPcieEngine (Engine)) {
    PcieTrainingSetPortStateV1 (Engine, LinkStateResetDuration, TRUE, Pcie);
    GnbLibPciRMW (
 ...
 */
VOID
PcieTrainingCheckResetDurationV1 (
  IN      PCIe_ENGINE_CONFIG    *CurrentEngine,
  IN      PCIe_PLATFORM_CONFIG  *Pcie
  )
{
  ...
  if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= Pcie->LinkGpioResetAssertionTime) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateResetExit, FALSE, Pcie);
  }
}
  ...
  )
{
  ...
  PcieTrainingSetPortStateV1 (CurrentEngine, LinkTrainingResetTimeout, TRUE, Pcie);
}
  ...
  )
{
  ...
  if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= Pcie->LinkResetToTrainingTime) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateReleaseTraining, FALSE, Pcie);
  }
}
  ...
  )
{
  ...
  PcieTrainingSetPortStateV1 (CurrentEngine, LinkTrainingState, TRUE, Pcie);
}
  ...
  )
{
  ...
  if (LinkHwStateHistory[0] > 4) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateDetecting, TRUE, Pcie);
    return;
  }
  TimeStamp = PcieTimerGetTimeStamp (Pcie);
  if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= Pcie->LinkReceiverDetectionPooling) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateDeviceNotPresent, FALSE, Pcie);
  }
}
  ...
  )
{
  ...
  if (LinkHwStateHistory[0] == 0x10) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateL0, FALSE, Pcie);
    return;
  };
  TimeStamp = PcieTimerGetTimeStamp (Pcie);
  if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= Pcie->LinkL0Pooling) {
    ...
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkTrainingState, FALSE, Pcie);
  }
}
...
{
  PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateVcoNegotiation, TRUE, Pcie);
}
/*----------------------------------------------------------------------------------------*/
/**
  ...
  )
{
  ...
  if (DxF0x128.Field.VcNegotiationPending == 0) {
    ...
    if (Pcie->GfxCardWorkaround == GfxWorkaroundEnable && NumberOfPhyLane >= 8) {
      ...
      PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateGfxWorkaround, TRUE, Pcie);
    } else {
      PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateTrainingSuccess, FALSE, Pcie);
    }
    return;
  }
  if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= 1000 * 1000) {
    PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateRetrain, FALSE, Pcie);
  }
}
  ...
  )
{
  ...
  case GFX_WORKAROUND_DEVICE_NOT_READY:
    if (TIMESTAMPS_DELTA (TimeStamp, CurrentEngine->Type.Port.TimeStamp) >= (3 * 1000000)) {
      PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateTrainingFail, TRUE, Pcie);
    }
    break;
  ...
  case GFX_WORKAROUND_RESET_DEVICE:
    if (CurrentEngine->Type.Port.GfxWrkRetryCount < 5) {
      ...
      PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateResetAssert, TRUE, Pcie);
    } else {
      PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateTrainingFail, TRUE, Pcie);
    }
    break;
  ...
  )
{
  GnbLibPciRMW (
    CurrentEngine->Type.Port.Address.AddressValue | DxF0x68_ADDRESS,
    AccessWidth32,
    (UINT32) ~DxF0x68_RetrainLink_MASK,
    1 << DxF0x68_RetrainLink_OFFSET,
    GnbLibGetHeader (Pcie)
    );
  PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateDetecting, TRUE, Pcie);
}
  ...
  )
{
  ...
  PcieTrainingSetPortStateV1 (CurrentEngine, LinkStateTrainingCompleted, FALSE, Pcie);
  GnbLibPciRMW (
    CurrentEngine->Type.Port.Address.AddressValue | DxF0x68_ADDRESS,
    AccessWidth32,
    (UINT32) ~DxF0x68_LinkBWManagementStatus_MASK,
    1 << DxF0x68_LinkBWManagementStatus_OFFSET,
    GnbLibGetHeader (Pcie)
    );
}
  ...
  )
{
  ...
  case LinkStateResetDuration:
    PcieTrainingCheckResetDurationV1 (Engine, Pcie);
    break;
  case LinkStateResetExit:
    PcieTrainingDeassertResetV1 (Engine, Pcie);
    break;
  ...
  case LinkStateGen2Fail:
    PcieTrainingGen2FailV1 (Engine, Pcie);
    break;
  case LinkStateL0:
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTrainingV1 Enter\n");
  do {
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTrainingV1 Exit [%x]\n", Status);
