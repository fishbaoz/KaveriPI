 ...
 * @file
 *
 * PCIe initialization at AMD_INIT_MID
 *
 *
 *
...
AGESA_STATUS
STATIC
PcieAcsCapabilityWrapperInitCallbackTN (
  IN       PCIe_WRAPPER_CONFIG           *Wrapper,
  IN OUT   VOID                          *Buffer,
  IN       PCIe_PLATFORM_CONFIG          *Pcie
  )
{
  PcieAcsCapabilityWrapperEnableV4 (Wrapper, Pcie);
  return AGESA_SUCCESS;
}
...
VOID
STATIC
PcieAcsCapabilityPortInitCallbackTN (
  IN       PCIe_ENGINE_CONFIG    *Engine,
  IN OUT   VOID                  *Buffer,
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  )
{
  PcieAcsCapabilityPortEnableV4 (Engine, Pcie);
}

/*----------------------------------------------------------------------------------------*/
/**
 * Master procedure to init Acs features
 *
 *
 *
 *
...
AGESA_STATUS
STATIC
PcieMidAcsCapEnabledTN (
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  )
{
  AGESA_STATUS          AgesaStatus;
  AGESA_STATUS          Status;
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidAcsCapEnabledTN Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidAcsCapEnabledTN Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
  ...
  )
{
  ...
  if (Status == AGESA_SUCCESS) {
    ...
    if (MidParamsPtr->GnbMidConfiguration.PcieAcsCapability == PcieAcsEnabled) {
      PcieMidAcsCapEnabledTN (Pcie);
    }
