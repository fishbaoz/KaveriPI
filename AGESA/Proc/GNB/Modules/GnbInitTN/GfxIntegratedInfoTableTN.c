 ...
 * @file
 *
 * Build Integrated Graphics System Information Table
 *
 *
 *
...
VOID
GfxIntInfoTableInitDispclkTableTN (
...
VOID
GfxIntInfoTableInitSclkTableTN (
  IN       PP_FUSE_ARRAY                    *PpFuseArray,
  IN       ATOM_INTEGRATED_SYSTEM_INFO_V1_7 *IntegratedInfoTable,
  IN       GFX_PLATFORM_CONFIG              *Gfx
  )
  ...
  )
{
  ...
  GfxPgfsmRegisterReadTN (PGFSM_DELAY_REG0_ADDRESS, &PgfsmDelayReg0, GnbLibGetHeader (Gfx));
  GfxPgfsmRegisterReadTN (PGFSM_DELAY_REG1_ADDRESS, &PgfsmDelayReg1, GnbLibGetHeader (Gfx));
  ResetTime = (PgfsmDelayReg0 & PGFSM_DELAY_REG0_ResetDelay_MASK) >> PGFSM_DELAY_REG0_ResetDelay_OFFSET;
  IsoTime = (PgfsmDelayReg0 & PGFSM_DELAY_REG0_IsolationDelay_MASK) >> PGFSM_DELAY_REG0_IsolationDelay_OFFSET;
  MemSd = (PgfsmDelayReg0 & PGFSM_DELAY_REG0_MemShutdownDelay_MASK) >> PGFSM_DELAY_REG0_MemShutdownDelay_OFFSET;
  MotherPso = (PgfsmDelayReg1 & PGFSM_DELAY_REG1_MotherDelay_MASK) >> PGFSM_DELAY_REG1_MotherDelay_OFFSET;
  DaughterPso = (PgfsmDelayReg1 & PGFSM_DELAY_REG1_DaughterDelay_MASK) >> PGFSM_DELAY_REG1_DaughterDelay_OFFSET;
  ...
  )
{
  ...
  SystemInfoTableV2.sIntegratedSysInfo.ulDentistVCOFreq = GfxLibGetSystemPllCofTN (GnbLibGetHeader (Gfx)) * 100;
  ...
  SystemInfoTableV2.sIntegratedSysInfo.sExtDispConnInfo.ucFixDPVoltageSwing = (UINT8) Gfx->DpFixedVoltSwingType;
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PCIe RefClk SS Percent : %d\n", Gfx->PcieRefClkSpreadSpectrum);
  ...
  if (Gfx->LvdsMiscControl.Field.LvdsVoltOverwriteEn) {
    SystemInfoTableV2.sIntegratedSysInfo.ucLVDSVoltAdjust = Gfx->LVDSVoltAdjust;
  ...
  if (PpFuseArray != NULL) {
    ...
    GfxIntInfoTableInitDispclkTableTN (PpFuseArray, &SystemInfoTableV2.sIntegratedSysInfo, Gfx);
    // Build Sclk info table
    GfxIntInfoTableInitSclkTableTN (PpFuseArray, &SystemInfoTableV2.sIntegratedSysInfo, Gfx);
  } else {
    Status = AGESA_ERROR;
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
  }
  ...
  GfxIntInfoTableInitTDPConfigTN (&SystemInfoTableV2.sIntegratedSysInfo, Gfx);
  ...
  GfxIntInfoTablePostToFb (&SystemInfoTableV2, Gfx);
