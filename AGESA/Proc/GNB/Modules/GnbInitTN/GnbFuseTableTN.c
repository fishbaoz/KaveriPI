...
PP_FUSE_ARRAY  DefaultPpFuseArrayTN = {
  0,                              // PP table revision
...
FUSE_REGISTER_ENTRY_TN D0F0xBC_xE0107067_TABLE [] = {
  {
    ...
    (UINT16) offsetof (PP_FUSE_ARRAY, DispClkDid[0])
  }
};
  ...
  )
{
  ...
  if (PpFuseArray != NULL) {
    ...
    if ((D18F3xA0.Field.CofVidProg) && (GnbBuildOptions.GnbLoadRealFuseTable)) {
      ...
      LibAmdMemCopy (PpFuseArray, &DefaultPpFuseArrayTN, sizeof (PP_FUSE_ARRAY), StdHeader);
