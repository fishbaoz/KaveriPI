 ...
 * @file
 *
 * Family-specific PCIe initialization at AMD_INIT_POST
 *
 *
 *
  ...
  )
{
  ...
  if (PcieConfigCheckPortStatus (Engine, INIT_STATUS_PCIE_TRAINING_SUCCESS) && (LinkSpeedCapability > PcieGen1) && !PcieConfigIsSbPcieEngine (Engine)) {
    PcieTrainingSetPortStateV1 (Engine, LinkStateRetrain, FALSE, Pcie);
  ...
  if (Engine->Type.Port.PortData.MiscControls.LinkComplianceMode == 0x1) {
    ...
    PcieTrainingSetPortStateV1 (Engine, LinkStateResetExit, FALSE, Pcie);
  }
}
  ...
  )
{
  ...
  PcieTrainingSetPortStateV1 (Engine, State, FALSE, Pcie);
}
...
/*----------------------------------------------------------------------------------------*/
/**
 * PCIe Post Early Init Interface
 *
 *
 *
