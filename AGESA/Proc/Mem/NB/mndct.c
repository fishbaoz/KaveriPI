...
{
  NBPtr->MemNTristateUnusedChipSelects (NBPtr);
  ...
  )
{
  ...
  for (k = 0; k < sizeof (PinType); k++) {
    ...
    for (i = 0; i < TabSize[k]; i++) {
      if ((NBPtr->DCTPtr->Timings.CsEnabled & TabPtr[i]) == 0) {
        Value |= (UINT8) (1 << i);
      }
    }
  ...
  )
{
  ...
  NBPtr->FamilySpecificHook[DllPDBypassTxRxEn] (NBPtr, NULL);
...
{
  CONST UINT8 Txp[] = {0xFF, 0xFF, 3, 3, 4, 4, 5, 6, 7, 8};
  CONST UINT8 Txpdll[] = {0xFF, 0xFF, 0xA, 0xA, 0xD, 0x10, 0x14, 0x17, 0x1A, 0x1D};
  UINT8 i;
  UINT8 TxpVal;
  UINT8 TxpdllVal;
  UINT16 Speed;
  ...
  )
{
  ...
  if (TargetNbPs < 4) {
    ...
    /// Check to see if NB P-states have been disabled. This should only be needed for
...
{
  STATIC CONST UINT8 Tckesr[] = {4, 4, 4, 5, 5, 6, 7, 7, 2};
  UINT8 Tck;
  ...
  // DDR-2400  5ns = 6nCK       max(3nCK, 6nCK)    + 1 = 7nCK + 1nCK = 7nCK
