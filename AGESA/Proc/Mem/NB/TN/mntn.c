  ...
  )
{
  ...
  if (!MemNIsIdSupportedTN (&(MemPtr->DiesPerSystem->LogicalCpuid))) {
    return FALSE;
  }
  ...
  )
{
  ...
  NBPtr->TotalBitTimes2DRdTraining = TOTAL_BIT_TIMES_2D_RD_TRAINING_TN;
  NBPtr->TotalBitTimes2DWrTraining = TOTAL_BIT_TIMES_2D_WR_TRAINING_TN;
  NBPtr->PhaseLaneMask = 0x3FFFF;
  NBPtr->MaxDiamondStep = 3;
  NBPtr->DiamondWidthRd = 16;
  NBPtr->DiamondWidthWr = 9;
  NBPtr->RdRolloverMultiple = 25;
  ...
  NBPtr->GetMemClkFreqInCurrentContext = MemNGetMemClkFreqInCurrentContextTN;
  ...
  NBPtr->InPhaseCompareRdDqs2DPattern = MemNInPhaseCompareRdDqs2DPatternUnb;
  NBPtr->Phase180CompareRdDqs2DPattern = MemN180CompareRdDqs2DPatternUnb;
  NBPtr->AgressorContinuousWrites = MemNAgressorContinuousWritesUnb;
  NBPtr->GetPrbs2dRdDqsSeed = MemNGetPrbs2dRdDqsSeedUnb;
  NBPtr->InitializeRdDqs2dVictimContinuousWrites = MemNInitializeRdDqs2dVictimContinuousWritesUnb;
  NBPtr->FinalizeRdDqs2dVictimContinuousWrites = MemNFinalizeRdDqs2dVictimContinuousWritesUnb;
  NBPtr->InitializeRdDqs2dVictimChipSelContinuousWrites = MemNInitializeRdDqs2dVictimChipSelContinuousWritesUnb;
  NBPtr->StartRdDqs2dVictimContinuousWrites = MemNStartRdDqs2dVictimContinuousWritesUnb;
  NBPtr->MemN2DRdDQSDataCollection = MemN2DRdDQSEyeRimSearchUnb;
  ...
  NBPtr->IsSupported[PerDimmAggressors2D] = TRUE;
  NBPtr->IsSupported[OptimizedPatternWrite2D] = TRUE;
  NBPtr->IsSupported[RxChnMntClksEn] = TRUE;
  ...
  )
{
  ...
  // Only initialize on TN processors, otherwise exit.
  GetLogicalIdOfCurrentCore (&LogicalId, &(MemPtr->StdHeader));
  if (!((LogicalId.Revision & AMD_F15_TN_ALL) != 0)) {
    return;
  }
  ...
  // ECC
  //
  RefPtr->EnableEccFeature = TRUE;

  //
  // Vref
  //
  RefPtr->ExternalVrefCtl = FALSE;

  //
  //Training Mode
  //
  RefPtr->ForceTrainMode = FORCE_TRAIN_AUTO;
  ...
  RefPtr->DimmTypeUsedInMixedConfig = DDR3_TECHNOLOGY;
}
  ...
  )
{
  BOOLEAN Retval;
  Retval = TRUE;
  if (!MemNIsIdSupportedTN (&(NBPtr->MemPtr->DiesPerSystem[NBPtr->MCTPtr->NodeId].LogicalCpuid))) {
    Retval = FALSE;
  }
  return Retval;
}
