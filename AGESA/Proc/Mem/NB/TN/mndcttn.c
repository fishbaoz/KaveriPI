  ...
  )
{
  ...
  if ((MCTPtr->LogicalCpuid.Revision & AMD_F15_TN_A0) != 0) {
  ...
  )
{
  ...
  for (j = 0; j < GET_SIZE_OF (SupportedFreq); j++) {
    if ((NBFreq * 100) > ((UINT32) SupportedFreq[j] * 125)) {
      DdrFreq = SupportedFreq[j];
      break;
    }
  }

  // Force DdrFreq to 333Mhz if NBFreq is 700Mhz
  if (NBFreq == 700) {
 ...
 */
UINT16
MemNGetMemClkFreqInCurrentContextTN (
  IN OUT   MEM_NB_BLOCK *NBPtr
  )
{
  UINT8 Dct;
