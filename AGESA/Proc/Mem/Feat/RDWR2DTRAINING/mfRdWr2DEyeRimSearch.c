  ...
  )
{
  ...
  if (HeapAllocateBuffer (&AllocHeapParams, &NBPtr->MemPtr->StdHeader) == AGESA_SUCCESS) {
    ...
    PutEventLog (AGESA_FATAL, MEM_ERROR_HEAP_ALLOCATE_FOR_2D, NULL, NULL, NULL, NULL, &NBPtr->MemPtr->StdHeader);
    return FALSE;
  }
  for (lane = 0; lane < MemFRdWr2DGetMaxLanes (NBPtr); lane++) {
  ...
  if (HeapDeallocateBuffer (AMD_MEM_2D_RD_WR_RIM_HANDLE, &NBPtr->MemPtr->StdHeader) != AGESA_SUCCESS) {
    SetMemError (AGESA_FATAL, NBPtr->MCTPtr);
    PutEventLog (AGESA_FATAL, MEM_ERROR_HEAP_DEALLOCATE_FOR_2D, NULL, NULL, NULL, NULL, &NBPtr->MemPtr->StdHeader);
  }
  return TRUE;
}
  ...
  )
{
  ...
  for (Lane = 0; Lane < MemFRdWr2DGetMaxLanes (NBPtr); Lane++) {
    SavedData->LaneSaved[Lane].Vref = &VrefPtr[Lane * NBPtr->TotalMaxVrefRange];
  }
  return FALSE;
}
