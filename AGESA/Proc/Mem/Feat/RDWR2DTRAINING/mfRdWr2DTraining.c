  ...
  )
{
  ...
  } else if (NBPtr->RefPtr->ForceTrainMode == FORCE_TRAIN_2D) {
    return TRUE;
  } else if (NBPtr->RefPtr->ForceTrainMode == FORCE_TRAIN_1D) {
    return FALSE;
  } else {
    ASSERT (FALSE);
  }
  ...
  )
{
  ...
  for (Lane = 0; Lane < MemFRdWr2DGetMaxLanes (NBPtr); Lane++) {
    ...
    if (Data->Lane[Lane].MaxRdWrDly < Data->MaxRdWrSweep) {
      ...
      // Limit DQS Delay to Zero
      Data->Lane[Lane].MaxRdWrDly = 0;
    }
  }
