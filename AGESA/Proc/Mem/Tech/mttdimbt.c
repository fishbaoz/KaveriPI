  ...
  for (ChipSelect = 0; ChipSelect < NBPtr->CsPerChannel; ChipSelect = ChipSelect + NBPtr->CsPerDelay) {
    if ((NBPtr->DCTPtr->Timings.CsPresent & ((UINT16) ((NBPtr->CsPerDelay == 2)? 3 : 1) << ChipSelect)) != 0) {
      ...
      for (ByteLane = 0; ByteLane < ((NBPtr->MCTPtr->Status[SbEccDimms] && NBPtr->IsSupported[EccByteTraining]) ? 9 : 8); ByteLane++) {
        ...
        // RxDly of DQS
        IDS_HDT_CONSOLE (MEM_FLOW, "\t RxDly DQS: Dimm %x, ByteLane %x\n", Dimm, ByteLane);
        DqsValue = MemNGetBitFieldNb (NBPtr, PER_DATA_BYTE (PAD_DQS_H, ByteLane, BitField));

        // RxDly of DQ_L
        IDS_HDT_CONSOLE (MEM_FLOW, "\t RxDly DQ_L: Dimm %x, ByteLane %x\n", Dimm, ByteLane);
        DqValue = MemNGetBitFieldNb (NBPtr, PER_DATA_BYTE (PAD_DQ_L, ByteLane, BitField));
        // Chipselect has the worst round trip.
        DqAndDqsDly = (UINT16) (DqValue + DqsValue);
        ...
        // RxDly of DQ_H
        IDS_HDT_CONSOLE (MEM_FLOW, "\t RxDly DQ_H: Dimm %x, ByteLane %x\n", Dimm, ByteLane);
        DqValue = MemNGetBitFieldNb (NBPtr, PER_DATA_BYTE (PAD_DQ_H, ByteLane, BitField));
        // Chipselect has the worst round trip.
        DqAndDqsDly = (UINT16) (DqValue + DqsValue);
