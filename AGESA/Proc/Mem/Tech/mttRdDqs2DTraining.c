  ...
  )
{
  ...
  if (HeapAllocateBuffer (&AllocHeapParams, &MemPtr->StdHeader) == AGESA_SUCCESS) {
    ...
    PutEventLog (AGESA_FATAL, MEM_ERROR_HEAP_ALLOCATE_FOR_2D, NULL, NULL, NULL, NULL, &MemPtr->StdHeader);
    return TRUE;
  }
  ...
  if (HeapDeallocateBuffer (AMD_MEM_2D_RDQS_HANDLE, &MemPtr->StdHeader) != AGESA_SUCCESS) {
    SetMemError (AGESA_FATAL, NBPtr->MCTPtr);
    PutEventLog (AGESA_FATAL, MEM_ERROR_HEAP_DEALLOCATE_FOR_2D, NULL, NULL, NULL, NULL, &MemPtr->StdHeader);
  }
  ...
  )
{
  ...
  } else if (TechPtr->NBPtr->RefPtr->ForceTrainMode == FORCE_TRAIN_2D) {
    return TRUE;
  } else if (TechPtr->NBPtr->RefPtr->ForceTrainMode == FORCE_TRAIN_1D) {
    return FALSE;
  } else {
    ASSERT (FALSE);
  }
  ...
  )
{
  ...
  if (TechPtr->NBPtr->RefPtr->ExternalVrefCtl == FALSE) {
    ...
    //
    /// None of the platform have external vRef control hence Agesa callout is not required to change external Vref
    //
  ...
  )
{
  ...
  if (status == FALSE) {
    SetMemError (AGESA_FATAL, NBPtr->MCTPtr);
    PutEventLog (AGESA_FATAL, MEM_ERROR_INVALID_2D_RDDQS_VALUE, NULL, NULL, NULL, NULL, &TechPtr->NBPtr->MemPtr->StdHeader);
  }
  return status;
}
/* -----------------------------------------------------------------------------*/
  ...
  )
{
  ...
  /// MaxRegVref. Don't need family specific hook since code moved to Feat\RDWR2DTRAINING\<FamilySpecific Folder> for newer families.
