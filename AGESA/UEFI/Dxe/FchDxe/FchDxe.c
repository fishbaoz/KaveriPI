  ...
  )
{
  ...
  Status = mBufferMgr->AmdBufferCallout (AGESA_LOCATE_BUFFER, FcnData, LocateParams);
  if (EFI_ERROR (Status)) {
    return AGESA_ERROR;
  }
  return Status;
  ...
  )
{
  ...
  Status = mBufferMgr->AmdBufferCallout (AGESA_ALLOCATE_BUFFER, FcnData, AllocParams);
  if (EFI_ERROR (Status)) {
    return AGESA_ERROR;
  }
  return Status;
  ...
  )
{
  ...
  Status = mBufferMgr->AmdBufferCallout (AGESA_DEALLOCATE_BUFFER, FcnData, DeallocParams);
  if (EFI_ERROR (Status)) {
    return AGESA_ERROR;
  }
  return Status;
...
// Driver Global Data
//
EFI_DRIVER_ENTRY_POINT (FchDxeInit);
