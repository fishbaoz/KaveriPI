...
#include "Filecode.h"
#define FILECODE UEFI_DXE_AMDFCHWHEA_AMDFCHWHEAHEST_FILECODE
...
AmdFchWheaInitEntryHest ( VOID )
{
  ...
  do {
    ...
    if (Table->Signature == 0x54534548) {
      ...
      EfiZeroMem (NewTable, MAX_HEST_SIZE);
      //
      // Copy Table
      //
  ...
  )
{
  ...
  EfiCopyMem (ErrSource, &mAmdErrSource, sizeof (ERROR_SOURCE_STRUCTURE_GENERIC_HW_ERROR));
