...
#include "FchPeiLib.h"
#include "Filecode.h"
#define FILECODE UEFI_PEI_FCHPEIAUX_PEIGPIO_FILECODE


/**
 * FchPeiAuxGpio - Program GPIO Service
 *
  ...
  )
{
  ...
  GpioPeiAuxPt = (GPIO_DEF*) GpioPr;
  for (i = 0; i < NumberOfGpio; i++) {
    LibFchMemRMW8 ( PeiServices, MmioAddr + IOMUX_BASE + GpioPeiAuxPt[i].Gpio, 0x00, GpioPeiAuxPt[i].GpioSelect);
    LibFchMemRMW8 ( PeiServices, MmioAddr + GPIO_BASE + GpioPeiAuxPt[i].Gpio, 0xA3, GpioPeiAuxPt[i].GpioValue);
    LibFchMemRMW8 ( PeiServices, MmioAddr + GPIO_BASE + GpioPeiAuxPt[i].Gpio, 0xDF, GpioPeiAuxPt[i].GpioType);
  }
}
