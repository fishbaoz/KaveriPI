...
#include EFI_PPI_CONSUMER (AmdResetRequestPpi)
#include EFI_PPI_CONSUMER (Reset)

// Produced PPIs
#include EFI_PPI_PRODUCER (AmdResetManagerPpi)
 ...
 *----------------------------------------------------------------------------------------
 */
EFI_PEIM_ENTRY_POINT (AmdResetManagerEntryPoint);
  ...
  )
{
  ...
  if (EFI_SUCCESS == Status) {
    ...
    ResetType = TempPpiPtr->ResetType;
  ...
  while (EFI_SUCCESS == Status) {
    ...
    if (EFI_SUCCESS == Status) {
      ...
      // check the reset requested type
      if (PeiResetCold == TempPpiPtr->ResetType) {
        ResetType = PeiResetCold;
      }
  ...
  for (i = 0; i < NumCallBack; i++) {
    Status = (ResetReqPpi[i]->Notify) (PeiServices);
  }
