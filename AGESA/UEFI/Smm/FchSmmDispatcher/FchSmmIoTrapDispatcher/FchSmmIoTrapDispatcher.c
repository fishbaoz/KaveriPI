...
EFI_STATUS
EFIAPI
FchSmmIoTrapDispatchUnRegister (
  IN       FCH_SMM_IO_TRAP_DISPATCH_PROTOCOL      *This,
  IN       EFI_HANDLE                             DispatchHandle
  );

FCH_SMM_IO_TRAP_DISPATCH_PROTOCOL gFchSmmIoTrapDispatchProtocol = {
  FchSmmIoTrapDispatchRegister,
  FchSmmIoTrapDispatchUnRegister
};

VOLATILE UINT32  AcpiMmioBase = ACPI_MMIO_BASE;
  ...
  for (Index = 0; Index < (sizeof (mFchIoTrapList) / sizeof (FCH_IO_TRAP_ENTRY)); Index++) {
    if (mFchIoTrapList[Index].DispatchFunction == NULL) {
      ...
      *DispatchHandle = &mFchIoTrapList[Index];
      ...
      ACPIMMIO32 (AcpiMmioBase + 0x290) = mFchIoTrapList[Index].StatusMask;
      //Set base address
      ACPIMMIO32 (AcpiMmioBase + 0x200 + mFchIoTrapList[Index].BaseAddressRegister) = RegisterContext->Address;
      //Set Read Write Attribute
      ACPIMMIO32 (AcpiMmioBase + 0x200 + mFchIoTrapList[Index].RwAddressRegister) &= 0x2;
      if (RegisterContext->Type == AmdWriteTrap) {
        ACPIMMIO32 (AcpiMmioBase + 0x200 + mFchIoTrapList[Index].RwAddressRegister) |= 0x2;
      }
      // Enable SMI
      ACPIMMIO32 (AcpiMmioBase + 0x2C4) |= (1 << mFchIoTrapList[Index].EnableBitOffset);
  ...
  )
{
  ...
  for (Index = 0; Index < (sizeof (mFchIoTrapList) / sizeof (FCH_IO_TRAP_ENTRY)); Index++) {
    if ( &mFchIoTrapList[Index] == DispatchHandle) {
      ...
      ACPIMMIO32 (AcpiMmioBase + 0x290) = mFchIoTrapList[Index].StatusMask;
      // Disable SMI
      ACPIMMIO32 (AcpiMmioBase + 0x2C4) &= (~(3 << mFchIoTrapList[Index].EnableBitOffset));
      Status = EFI_SUCCESS;
    }
  }
  return  Status;
