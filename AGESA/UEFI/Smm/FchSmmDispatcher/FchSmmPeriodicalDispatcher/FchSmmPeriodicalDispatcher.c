...
EFI_STATUS
EFIAPI
FchSmmPeriodicalDispatchRegister (
...
EFI_STATUS
EFIAPI
FchSmmPeriodicalDispatchUnRegister (
  IN       CONST FCH_SMM_PERIODICAL_DISPATCH_PROTOCOL   *This,
  IN       EFI_HANDLE                                   DispatchHandle
  );

FCH_SMM_PERIODICAL_DISPATCH_PROTOCOL gFchSmmPeriodicalDispatchProtocol = {
  FchSmmPeriodicalDispatchRegister,
  FchSmmPeriodicalDispatchUnRegister
};
  ...
  )
{
  ...
  if (HeadFchSmmPeriodicalNodePtr->FchPeriodicalNodePtr == NULL) {
    ...
    while (CurrentFchSmmPeriodicalNodePtr->FchPeriodicalNodePtr!= NULL) {
      ...
      Status = CurrentFchSmmPeriodicalNodePtr->CallBackFunction (
  ...
  )
{
  ...
  if (HeadFchSmmPeriodicalNodePtr->CallBackFunction != NULL) {
    return Status;
  } else {
    ...
    NewFchSmmPeriodicalNodePtr->CallBackFunction = CallBackFunction;
    *DispatchHandle = &NewFchSmmPeriodicalNodePtr->DispatchHandle;
    ...
    if (NewFchSmmPeriodicalNodePtr->Context.AmdSmiTimer1m == 1) {
      ACPIMMIO32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG98) |= BIT29;
    } else {
      ACPIMMIO32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG98) &= ~(BIT29);
    }
    ACPIMMIO16 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG96) = NewFchSmmPeriodicalNodePtr->Context.AmdSmiTimerDuration;
    return  Status;
  }
}
