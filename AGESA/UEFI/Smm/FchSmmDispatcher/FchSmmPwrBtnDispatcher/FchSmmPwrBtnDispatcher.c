...
EFI_STATUS
EFIAPI
FchSmmPwrBtnDispatchUnRegister (
  IN       CONST FCH_SMM_PWRBTN_DISPATCH_PROTOCOL   *This,
  IN       EFI_HANDLE                               DispatchHandle
  );

FCH_SMM_PWRBTN_DISPATCH_PROTOCOL gFchSmmPwrBtnDispatchProtocol = {
  FchSmmPwrBtnDispatchRegister,
  FchSmmPwrBtnDispatchUnRegister
};
  ...
  )
{
  ...
  if (HeadFchSmmPwrBtnNodePtr->FchPwrBtnNodePtr == NULL) {
    ...
    while (CurrentFchSmmPwrBtnNodePtr->FchPwrBtnNodePtr!= NULL) {
      if (CurrentFchSmmPwrBtnNodePtr->Context.Phase == AmdPowerButtonEntry) {
        if (CurrentFchSmmPwrBtnNodePtr->CallBackFunction != NULL) {
          Status = CurrentFchSmmPwrBtnNodePtr->CallBackFunction (
    ...
    CurrentFchSmmPwrBtnNodePtr = HeadFchSmmPwrBtnNodePtr;
    while (CurrentFchSmmPwrBtnNodePtr->FchPwrBtnNodePtr!= NULL) {
      if (CurrentFchSmmPwrBtnNodePtr->Context.Phase == AmdPowerButtonExit) {
        if (CurrentFchSmmPwrBtnNodePtr->CallBackFunction != NULL) {
          Status = CurrentFchSmmPwrBtnNodePtr->CallBackFunction (
                                               CurrentFchSmmPwrBtnNodePtr->DispatchHandle,
                                               &CurrentFchSmmPwrBtnNodePtr->Context
  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status);
  NewFchSmmPwrBtnNodePtr->CallBackFunction = CallBackFunction;
  NewFchSmmPwrBtnNodePtr->CallBack2Function = NULL;
...
/*----------------------------------------------------------------------------------------*/
/**
 * Register POWER BOTTON child handler2
 *
 *
 * @param[in]       This                  Pointer to protocol
  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status);
  NewFchSmmPwrBtnNodePtr->CallBackFunction = NULL;
  NewFchSmmPwrBtnNodePtr->CallBack2Function = CallBack2Function;
  *DispatchHandle = &NewFchSmmPwrBtnNodePtr->DispatchHandle;
  NewFchSmmPwrBtnNodePtr->DispatchHandle = *DispatchHandle;
  NewFchSmmPwrBtnNodePtr->Context = *RegisterContext;
...
/*----------------------------------------------------------------------------------------*/
/**
 * Unregister POWER BUTTON child handler2
 *
 *
 * @param[in]       This                  Pointer to protocol
 * @param[in]       DispatchHandle
