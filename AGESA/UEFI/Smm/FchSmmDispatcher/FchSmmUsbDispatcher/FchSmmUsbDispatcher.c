...
EFI_STATUS
EFIAPI
FchSmmUsbDispatchRegister (
  IN       CONST FCH_SMM_USB_DISPATCH_PROTOCOL   *This,
  IN       FCH_SMM_USB_HANDLER_ENTRY_POINT       CallBackFunction,
  IN OUT   FCH_SMM_USB_REGISTER_CONTEXT          *RegisterContext,
  OUT      EFI_HANDLE                            *DispatchHandle
  );

EFI_STATUS
EFIAPI
FchSmmUsbDispatchUnRegister (
  IN       CONST FCH_SMM_USB_DISPATCH_PROTOCOL   *This,
  IN       EFI_HANDLE                            DispatchHandle
  );

FCH_SMM_USB_DISPATCH_PROTOCOL gFchSmmUsbDispatchProtocol = {
  FchSmmUsbDispatchRegister,
  FchSmmUsbDispatchUnRegister
};
  ...
  )
{
  ...
  if (HeadFchSmmUsbNodePtr->FchUsbNodePtr == NULL) {
    ...
    while (CurrentFchSmmUsbNodePtr->FchUsbNodePtr!= NULL) {
      if (CurrentFchSmmUsbNodePtr->CallBackFunction != NULL) {
           Status = CurrentFchSmmUsbNodePtr->CallBackFunction (
  ...
  )
{
  ...
  NewFchSmmUsbNodePtr->CallBackFunction = CallBackFunction;
  NewFchSmmUsbNodePtr->CallBack2Function = NULL;
  ...
  if (HeadFchSmmUsbNodePtr->FchUsbNodePtr == NULL) {
    ...
    while (CurrentFchSmmUsbNodePtr->FchUsbNodePtr != NULL) {
      if (NewFchSmmUsbNodePtr->Context.Order <= CurrentFchSmmUsbNodePtr->Context.Order &&
          ...
          EfiCompareMem (CurrentFchSmmUsbNodePtr->Context.Device, NewFchSmmUsbNodePtr->Context.Device, DevicePathSize (NewFchSmmUsbNodePtr->Context.Device)) == 0) {
  ...
  )
{
  ...
  NewFchSmmUsbNodePtr->CallBackFunction = NULL;
  NewFchSmmUsbNodePtr->CallBack2Function = CallBack2Function;
  NewFchSmmUsbNodePtr->Context = *RegisterContext;
  *DispatchHandle = &NewFchSmmUsbNodePtr->DispatchHandle;
  NewFchSmmUsbNodePtr->DispatchHandle = *DispatchHandle;
