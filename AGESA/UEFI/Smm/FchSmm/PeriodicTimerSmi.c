...
AmdSmiLongTimerCallback (
  ...
  IN  FCH_SMM_PERIODICAL_REGISTER_CONTEXT      *DispatchContext
  )
{
  LibAmdIoWrite (AccessWidth16, 0x80, &mLongTimerCounter, NULL);
  mLongTimerCounter++;
  return EFI_SUCCESS;
}
...
{
  LibAmdIoWrite (AccessWidth16, 0x80, &mShortTimerCounter, NULL);
  mShortTimerCounter++;
  return EFI_SUCCESS;
}
...
{
  EFI_STATUS                              Status;
  FCH_SMM_PERIODICAL_DISPATCH_PROTOCOL    *AmdPeriodicalDispatch;
  ...
  if (FchMisc->LongTimer.Enable) {
    PeriodicalRegisterContext.AmdSmiTimer1m  = 1;
    PeriodicalRegisterContext.AmdSmiTimerDuration = FchMisc->LongTimer.CycleDuration & ~SMI_TIMER_ENABLE;
    if (FchMisc->LongTimer.StartNow) {
      PeriodicalRegisterContext.AmdSmiTimerDuration |= SMI_TIMER_ENABLE;
    }
    mLongTimerCounter = 0;
    ...
    PeriodicalRegisterContext.AmdSmiTimer1m  = 0;
    PeriodicalRegisterContext.AmdSmiTimerDuration = (FchMisc->ShortTimer.CycleDuration >> 1) & ~SMI_TIMER_ENABLE;
    if (FchMisc->ShortTimer.StartNow) {
      PeriodicalRegisterContext.AmdSmiTimerDuration |= SMI_TIMER_ENABLE;
    }
