...
{
  EFI_STATUS                                 Status;
  FCH_SMM_IO_TRAP_DISPATCH_PROTOCOL          *AmdIoTrapDispatch;
  ...
  )
{
  ...
  //do USB reset
  bData = 0x87;
  WritePmio (0xF3, AccessWidth8, &bData, NULL);
  FchStall (5, NULL);          //wait
  bData = 0x00;
  WritePmio (0xF3, AccessWidth8, &bData, NULL);

  //enable CF9
  RwPmio (0xD2, AccessWidth8, ~BIT6, 0, NULL);

  //reset system by CF9
  LibAmdIoRead (AccessWidth8, 0xCF9, &bData, NULL);
  bData |= BIT2;
  LibAmdIoWrite (AccessWidth8, 0xCF9, &bData, NULL);
//  WritePMIO (0xC5, AccessWidth8, &bData);
