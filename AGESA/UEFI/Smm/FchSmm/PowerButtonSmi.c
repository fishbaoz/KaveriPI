...
{
  EFI_STATUS                              Status;
  FCH_SMM_PWRBTN_DISPATCH_PROTOCOL        *AmdPwrBtnDispatch;
  FCH_SMM_PWRBTN_DISPATCH2_PROTOCOL       *AmdPwrBtnDispatch2;
...
//
  ...
  AmdPowerButtonContext.Phase = AmdPowerButtonExit;
  AmdPowerButtonContext.Order = 0x80;
  Status = AmdPwrBtnDispatch2->Register (
                                AmdPwrBtnDispatch2,
...
//
  ...
  AmdPowerButtonContext.Phase = AmdPowerButtonEntry;
...
FchEnableSleepTransition ( VOID )
{
  ...
  RwPmio (FCH_PMIOA_REG08 + 3, AccessWidth8, ~(BIT1 + BIT0), BIT0, NULL);
}
  ...
  )
{
  ...
  RwMem (AcpiMmioBase + SMI_BASE + FCH_SMI_REGB0, AccessWidth32, ~(BIT3 + BIT2), 0);

  // Clear power button status
  Or16 = BIT8;
  Mask16 = 0;
  LibAmdIoRMW (AccessWidth16, AcpiPmbase + R_FCH_ACPI_PM1_STATUS, &Or16, &Mask16, NULL);

  // power the system off now
  LibAmdIoRead (AccessWidth16, AcpiPmbase + R_FCH_ACPI_PM_CONTROL, &Value16, NULL);
  Value16 &= ~(BIT12 + BIT11 + BIT10);
  Value16 |= 5 << 10;
  LibAmdIoWrite (AccessWidth16, AcpiPmbase + R_FCH_ACPI_PM_CONTROL, &Value16, NULL);
  Value16 |= BIT13;
  LibAmdIoWrite (AccessWidth16, AcpiPmbase + R_FCH_ACPI_PM_CONTROL, &Value16, NULL);
