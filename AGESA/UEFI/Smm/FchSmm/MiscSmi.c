...
{
  EFI_STATUS                                Status;
  FCH_SMM_USB_DISPATCH_PROTOCOL             *FchSmmUsbDispatchProtocol;
  FCH_SMM_USB_REGISTER_CONTEXT              FchSmmUsbRegisterContext;
  EFI_HANDLE                                Handle;
  FCH_DATA_BLOCK                            *FchDb;
  ...
  if (FchDb->Usb.Xhci0Enable) {
    Status = gBS->LocateProtocol (
                    &gFchSmmUsbDispatchProtocolGuid,
                    NULL,
                    &FchSmmUsbDispatchProtocol
                    );

    if (!EFI_ERROR (Status)) {
      FchSmmUsbRegisterContext.Order  = 0x00;
      FchSmmUsbRegisterContext.Device = 0x00;
      FchSmmUsbRegisterContext.Type   = 0x00;

      FchSmmUsbDispatchProtocol->Register (FchSmmUsbDispatchProtocol,
                                            (FCH_SMM_USB_HANDLER_ENTRY_POINT)FchUsbBeforeOCcallback,
                                            &FchSmmUsbRegisterContext,
                                            &Handle);
    }
  }
  ...
  )
{
  ...
  for (Controller = 0; Controller < 2; Controller++) {
    ReadPci (((USB_XHCI_BUS_DEV_FUN + Controller) << 16) + 0x00 , AccessWidth32, &RegDword, &StdHeader);
    if (RegDword != 0xffffffff) {
      //Get BAR address
      ReadPci (((USB_XHCI_BUS_DEV_FUN + Controller) << 16) + 0x10 , AccessWidth32, &XhcBar, &StdHeader);
      XhcBar &= 0xfffffff0;
      //Get xECP
      RegDword = ACPIMMIO32 (XhcBar + 0x10); //HCCPARAMS
      RegDword &= 0xffff0000;
      xECP = XhcBar + (RegDword >> 14);

      //Check OC SMI
      if ( ACPIMMIO32 (xECP + 0x04) & BIT29) {
        for (Port = 0; Port < 2; Port++) {
          PORTSC = ACPIMMIO32 (XhcBar + 0x440 + 0x10 * Port);
          if ((PORTSC & BIT1) && ((PORTSC & 0x1E0) == 0x0)) {
            PORTSC &= ~(UINT32)BIT1;
            PORTSC = (PORTSC & 0xffffff0f) | 0x60;
            PORTSC |= BIT16;
            ACPIMMIO32 (XhcBar + 0x440 + 0x10 * Port) = PORTSC;
            Timer = 10;
            do {
              FchStall (3*1000, &StdHeader);
              Timer--;
            } while ( (Timer != 0) && ((ACPIMMIO32 (XhcBar + 0x440 + 0x10 * Port) & 0x1E0) != 0x60));
          }
        }
      }
    }
