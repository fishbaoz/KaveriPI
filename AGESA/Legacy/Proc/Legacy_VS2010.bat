...
echo off
echo **************************************************************************
echo * Legacy_VS2010.bat
echo *     This batchfile establishes the build environment for an AGESA(TM)
echo *     legacy build.
echo *
echo * Syntax:
echo *           Legacy_VS2010.bat   Solution  [DevTipPath]  [DevKey] [DoxKey] [Release^|Debug] [Internal]
echo * Parameters:
...
:DoLoop
  ...
  IF  /I "%1"=="RichlandFp2"    goto Case002
  IF  /I "%1"=="RichlandFs1r2"  goto Case002
  IF  /I "%1"=="RichlandFm2"    goto Case002
  IF  /I "%1"=="KaveriFm2r2"    goto Case002
  IF  /I "%1"=="KaveriFp3"      goto Case002
  IF  /I "%1"=="KabiniFt3"      goto Case002
  IF  /I "%1"=="KabiniFs1b"     goto Case002
  IF  /I "%1"=="MullinsFt3b"    goto Case002
  IF  /I "%1"=="KyotoFt3"       goto Case002
  IF  /I "%1"=="CarrizoFm2r2"   goto Case002
  IF  /I "%1"=="CarrizoFp4"     goto Case002
  ...
  :Case004
      ...
      :Else040
        ...
        :Else042
          ...
          Echo  .    .    .  Legacy_VS2010.bat  Solution  [DevTipPath]  [DevKey]  [DoxKey]
          exit /B 4
        :EndIf042
      :EndIf040
      goto EndSwitch
...
rem # Ensure that Microsoft Visual Studio 2010 is installed on this machine.
IF NOT "%VS100COMNTOOLS%"=="" goto EndIf95
echo ---------------------------------------------------------------------
echo -   Building AGESA ARCH2008 requires Microsoft Visual Studio 2010   -
echo -         Install Visual Studio 2010 and confirm environment        -
echo -         variable VS100COMNTOOLS is present.                        -
echo ---------------------------------------------------------------------
pause
exit
:EndIf95
...
IF NOT "%DevKey%"=="DevKey"  goto Else140
  ...
  start /NORMAL devenv.exe  %AGESA_ROOT%Legacy\Proc\%Solution%%SolutionType%Legacy_VS2010.sln /ProjectConfig "%Configuration%|Win32"
  goto EndIf140
:Else140
  rem Else140 - This is the command to perform a background or automated build ...
  devenv  %AGESA_ROOT%Legacy\Proc\%Solution%%SolutionType%Legacy_VS2010.sln /Rebuild "%Configuration%|Win32"
:EndIf140
:Exit
exit /B 0
